        ...
        <div class='h-full w-full text-white overflow-y-auto overflow-x-hidden'>
            <div class='grid-cols-1 md:grid-cols-3 grid gap-4 p-5'>
                <div class='flex flex-col gap-5 col-span-2 overflow-hidden h-full'>
                    <button class='w-max hover:bg-white hover:text-black/80 transition bg-white/20 rounded-full flex gap-2 items-center px-2 py-1' onClick={() => props.setViewProject(null)}><Icon path={mdiArrowLeft} size={1}/> {t('goback')}</button>
                ...
                <div class='flex flex-col gap-5 items-end bg-white/10 h-max rounded-md p-5'>
                    ...
                    {props.project.executable ? 
                        <button class='hover:bg-purple-500 w-full transition bg-blue-500/50 border-2 border-blue-500 rounded-full flex gap-2 items-center px-2 py-1' href={props.project.link} target={"_blank"}
